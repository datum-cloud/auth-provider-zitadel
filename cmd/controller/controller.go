...
import (
	...
	machineaccountkeys "go.miloapis.com/auth-provider-zitadel/pkg/machine-account-keys"
...
and manages the auth provider lifecycle.`,
	...
	cmd.Flags().DurationVar(&cfg.Zitadel.AccessTokenDuration, "zitadel-access-token-duration", 1*time.Hour, "The duration of the access token. Default is 1 hour.")
	...
	cmd.Flags().BoolVar(&cfg.EnableLeaderElection, "leader-elect", false, "Enable leader election for controller manager. "+"Enabling this will ensure there is only one active controller manager.")
...
func runController(cfg *config.ControllerConfig, globalConfig *config.GlobalConfig) error {
	...
	accesToken, err := machineaccountkeys.GetAccessToken(
		cfg.Zitadel.MachineAccountKeyPath,
		cfg.Zitadel.BaseURL,
		"urn:zitadel:iam:org:project:id:zitadel:aud",
		cfg.Zitadel.AccessTokenDuration,
	)
	if err != nil {
		setupLog.Error(err, "unable to get Zitadel access token")
		os.Exit(1)
	}
	zitadelHtppClient := zitadelHtppClient.NewClient(cfg.Zitadel.BaseURL, accesToken, "Bearer")
